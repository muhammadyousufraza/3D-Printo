...
( function( window, document, $, app, undefined ) {
	...
	app.SearchView = window.Backbone.View.extend({
		...
		selectPost: function( evt ) {
			...
			var page_current_g = $('.attached-posts-wrap.haru-search-post-in-group');
			if(page_current_g.length > 0){
				$lastRow = page_current_g.find( '.retrieved li' ).last();
			}
	...
	app.openSearch = function( $button ) {
		...
		$('.attached-posts-wrap').removeClass('haru-search-post-in-group');
		if($('.attached-posts-wrap').length > 1){
			var page_parent_g = $button.closest('.attached-posts-wrap');
			if(page_parent_g.length > 0){
				page_parent_g.addClass('haru-search-post-in-group');
			}
		}
