...
if ( ! class_exists( 'Haru_CMB_Conditionals', false ) ) {

	/**
	 * Haru_CMB_Conditionals Plugin.
	 */
	class Haru_CMB_Conditionals {
		...
		public function admin_footer() {
			// enqueue on editor
			$enqueue_script = in_array( $GLOBALS['pagenow'], array( 'post-new.php', 'post.php', 'admin.php' ), true ); // @custom
		...
		public function filter_data_to_save( CMB2 $cmb2, $object_id ) {
			foreach ( $cmb2->prop( 'fields' ) as $field_args ) {
				if ( class_exists('RankMath') && isset( $_POST['action'] ) && ( ! in_array( $_POST['action'], array( 'rank-math-options-titles', 'rank-math-options-general' ) ) ) ) {
	...
	if ( ! function_exists( 'cmb2_conditionals_init' ) ) {
		...
		function cmb2_conditionals_init() {
			...
			if ( null === $cmb2_conditionals ) {
				$cmb2_conditionals = new Haru_CMB_Conditionals();
			}
