jQuery( document ).ready( function( $ ) {
	...
	function CMB2ConditionalsInit( changeContext, conditionContext ) {
		...
		if ( 'undefined' === typeof changeContext || $( changeContext ).length < 1 ) { // @custom
			changeContext = 'body';
		}
		changeContext = $( changeContext );

		if ( 'undefined' === typeof conditionContext || $( changeContext ).length < 1 ) { // @custom
	...
	switch( true ) {
		...
		case $( '#post' ).length > 0:
			if ( $('body.post-type-product').length <= 0 ) {
				...
				// CMB2ConditionalsInit( '#post', '#post .cmb2-wrap', '#post .cmb2-postbox' );
				CMB2ConditionalsInit( '#post .cmb2-wrap' );
			}
			break;
