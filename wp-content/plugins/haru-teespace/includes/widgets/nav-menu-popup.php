...
if ( ! class_exists( 'Haru_TeeSpace_Nav_Menu_Popup_Widget' ) ) {
	class Haru_TeeSpace_Nav_Menu_Popup_Widget extends Widget_Base {

		protected $nav_menu_index = 1;

		public function get_name() {
			return 'haru-nav-menu-popup';
		}

		public function get_title() {
			return esc_html__( 'Haru Nav Menu Popup', 'haru-teespace' );
		}
		...
		protected function register_controls() {
			...
			$this->add_responsive_control(
                ...
                [
					...
					'selectors' => [
						'{{WRAPPER}} .haru-menu-toggle-popup' => '{{VALUE}}',
					]
                ]
            );
			...
			$this->add_control(
				...
				[
					'raw' => __( 'You can set style of Popup Menu here' ),
					'type' => Controls_Manager::RAW_HTML,
			...
			$this->add_responsive_control(
				...
				[
					...
					'default' => '',
					'selectors' => [
						'{{WRAPPER}} .haru-menu-toggle-popup' => 'color: {{VALUE}}',
					],
					'separator' => 'before',
				]
			);
			...
			$this->add_responsive_control(
				...
				[
					...
					'default' => '',
					'selectors' => [
						'{{WRAPPER}} .haru-menu-toggle-popup' => 'background-color: {{VALUE}}',
					],
				]
			);
		...
		protected function render() {
	        ...
	        $args = array(
	        	'echo' 				=> false,
                'menu' 				=> $settings['menu'],
                'menu_class' 		=> 'haru-nav-menu-popup',
			...
			$this->add_render_attribute( 'menu-toggle-popup', [
				'class' 		=> 'haru-menu-toggle-popup',
				'role' 			=> 'button',
				'tabindex' 		=> '0',
			...
			if ( ! empty( $settings['el_class'] ) ) {
				...
				$this->add_render_attribute( 'menu-toggle-popup', 'class', $settings['el_class'] );
			}

			$this->add_render_attribute( 'menu-toggle-popup', 'data-effect', 'mfp-zoom-in2' );
			$this->add_render_attribute( 'menu-toggle-popup', 'data-id', 'haru-nav-menu-popup-' . $this->get_id() );

			$this->add_render_attribute( 'menu-nav', 'class', 'haru-nav-menu-popup--dropdown' );
			$this->add_render_attribute( 'menu-nav', 'class', 'haru-nav-menu-popup__container' );
			...
			if ( $settings['layout'] == 'popup' ) {
				$this->add_render_attribute( 'haru-nav-menu-popup-wrap', 'class', 'white-popup-block mfp-hide mfp-with-anim' );
				$this->add_render_attribute( 'haru-nav-menu-popup-wrap', 'id', 'haru-nav-menu-popup-' . $this->get_id() );
			}
    		...
    		<?php endif; ?>
				<div <?php echo $this->get_render_attribute_string( 'menu-toggle-popup' ); ?>>
					<?php if ( $has_icon ) : ?>
			...
			<div <?php echo $this->get_render_attribute_string( 'haru-nav-menu-popup-wrap' ); ?>>
				<?php if ( $settings['menu_content'] == 'menu' ) : ?>
					<nav <?php echo $this->get_render_attribute_string( 'menu-nav' ); ?>>
						<?php echo $popup_menu_html; ?>
					</nav>
