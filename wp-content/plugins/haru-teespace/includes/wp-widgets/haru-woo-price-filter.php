...
class Haru_Woo_Price_Filter_Widget extends Haru_TeeSpace_Widget {
    ...
    public function __construct() {
        $this->widget_id          = 'haru_widget_woo_price_filter';
        $this->widget_name        = esc_html__( 'Haru WooCommerce Price Filter', 'haru-teespace' );
        $this->widget_description = esc_html__( 'Widget filter products by price range list.', 'haru-teespace' );
        $this->widget_cssclass    = 'widget-woo-price-filter';
        $this->cached             = false;

        $this->settings = array(
            'title'         => array(
                'type'  => 'text',
                'std'   => esc_html__( 'Price filter', 'haru-teespace' ),
                'label' => esc_html__( 'Title', 'haru-teespace' )
            ),
            'show_ranges' => array(
                'type'  => 'checkbox',
                'std'   => esc_html__( 'Price filter', 'haru-teespace' ),
                'label' => esc_html__( 'Show empty price ranges', 'haru-teespace' ),
                'description' => esc_html__( 'If you have a lot of products, it may increase this widget performance.', 'haru-teespace' ),
            ),
        );

        $this->_check_ranges = apply_filters( 'haru_check_ranges_price_filter', true );
    ...
    public function widget( $args, $instance ) {
        ...
        <?php
            ...
            if ( ! empty( $links ) ) {
                ?>
                    <div class="haru-price-filter">
                        <?php $scroll_for_widget = haru_get_option( 'haru_archive_product_widget_scroll', true ); ?>
                        <ul class="<?php echo ( $scroll_for_widget == true ) ? 'haru-scroll-widget' : ''; ?>">
                            <?php foreach ( $links as $link ) : ?>
                                <li>
                                    <a rel="nofollow noopener" href="<?php echo esc_url( $link['href'] ); ?>" class="<?php echo esc_attr( $link['class'] ); ?>"><?php echo wp_kses( $link['title'], 'post' ); ?></a>
                                </li>
                            <?php endforeach; ?>
                        </ul>
                    </div>
    ...
    private function generate_price_links( $min, $max, $min_price, $max_price, $show_ranges ) {
        ...
        $links[] = array(
            'href'  => $link_no_price,
            'title' => esc_html__( 'All', 'haru-teespace' ),
...
if ( ! function_exists( 'haru_register_widget_woo_price_filter' ) ) {
    function haru_register_widget_woo_price_filter() {
        if ( class_exists( 'WooCommerce', true ) ) {
            register_widget( 'Haru_Woo_Price_Filter_Widget' );
        }
    }

    add_action( 'widgets_init', 'haru_register_widget_woo_price_filter' );
}
