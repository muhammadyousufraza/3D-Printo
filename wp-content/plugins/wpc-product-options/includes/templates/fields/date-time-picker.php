...
<div class="wpcpo-item-line">
    ...
    <select name="<?php echo esc_attr( 'wpcpo-fields[' . $this->field_id . '][date_format]' ); ?>">
        <option value="F j, Y" <?php selected( $this->get_field_value( 'date_format' ), 'F j, Y' ); ?>><?php echo wp_date( 'F j, Y' ); ?></option>
        <option value="Y-m-d" <?php selected( $this->get_field_value( 'date_format' ), 'Y-m-d' ); ?>><?php echo wp_date( 'Y-m-d' ); ?></option>
        <option value="m/d/Y" <?php selected( $this->get_field_value( 'date_format' ), 'm/d/Y' ); ?>><?php echo wp_date( 'm/d/Y' ); ?></option>
        <option value="d/m/Y" <?php selected( $this->get_field_value( 'date_format' ), 'd/m/Y' ); ?>><?php echo wp_date( 'd/m/Y' ); ?></option>
    </select>
</div>
<div class="wpcpo-item-line">
    <label><?php esc_html_e( 'Time format', 'wpc-product-options' ); ?></label>
    <select name="<?php echo esc_attr( 'wpcpo-fields[' . $this->field_id . '][time_format]' ); ?>">
        <option value="g:i a" <?php selected( $this->get_field_value( 'time_format' ), 'g:i a' ); ?>><?php echo wp_date( 'g:i a' ); ?></option>
        <option value="g:i A" <?php selected( $this->get_field_value( 'time_format' ), 'g:i A' ); ?>><?php echo wp_date( 'g:i A' ); ?></option>
        <option value="H:i" <?php selected( $this->get_field_value( 'time_format' ), 'H:i' ); ?>><?php echo wp_date( 'H:i' ); ?></option>
    </select>
</div>
<div class="wpcpo-item-line">
    <label>
